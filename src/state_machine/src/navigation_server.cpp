...
public:
    ...
    navigation_server(ros::NodeHandle nh) : nh_(nh), priv_nh_("~")
    {
        ...
        srv_ = nh_.advertiseService("goto_table", &navigation_server::gototable, this);
...
void navigation_server::localization()
{
    ros::NodeHandle np;
    ros::Rate rp(60);
...
bool navigation_server::movebase(final_msg_srv::desired_positionRequest &req, final_msg_srv::desired_positionResponse &res)
{
    ...
    if (!number)
    navigation_server::localization();
    ROS_WARN_STREAM(__LINE__);
...
bool navigation_server::gototable(std_srvs::EmptyResponse &req, std_srvs::EmptyResponse &res)
{
    ros::NodeHandle np;
    ros::Rate rp(60);
    int count = 0;
